...
```{r multiplot_richness_three-taxa_BP_Chla_for_paper_2, echo=FALSE, fig.cap="", fig.width=5, fig.height=10}
...
labelheight.rich.obs <- extractmax.rich.obs*0.85
...
g.rich.obs <-ggplot(sddata, aes(x=SamplingDate)) +
  ...
  annotate("text", label="C", x=as.Date('2013-07-10'), y=labelheight.rich.obs, size=4) +
    theme_classic() +
...
extractmax.leucine <- max(sddata$mean.leu, na.rm=TRUE)
labelheight.leucine <- extractmax.leucine*0.85

g.leucine <- ggplot(sddata[!is.na(sddata$mean.leu),], aes(x=SamplingDate)) +
    ...
    xlab("") + 
    ylab(expression(paste("Production (pmol ", hour^{-1},")"))) +
   scale_x_date(date_breaks="1 month", 
                     date_labels="%m/%d",
                 limits = c(min,max)) +
  annotate("rect", 
           xmin=bloom.min, xmax=bloom.max, 
           ymin=-Inf,ymax=Inf, alpha = .1) +
  annotate("text", label="B", x=as.Date('2013-07-10'), y=labelheight.leucine, size=4) +
    theme_classic() +
    theme(text = element_text(size=8),
        legend.text=element_text(size=8),
        axis.line.x = element_line(color = "black"), 
...
extractmax.chl <- max(sddata$mean.chl, na.rm=TRUE)
labelheight.chl <- extractmax.chl*0.85

g.chl <-ggplot(sddata, aes(x=SamplingDate)) +
    geom_point(aes(y=mean.chl), shape=20, na.rm=T, size=1.5, colour="black") +    
    geom_errorbar(aes(ymin=mean.chl-se.chl, ymax=mean.chl+se.chl), 
                  width=.3, colour="black") +
    geom_line(aes(y=mean.chl), size=0.5, colour="black") + 
    xlab("") + 
...
panel.chl <- ggplot_gtable(ggplot_build(g.chl))
...
grid.arrange(panel.chl, panel.leu, panel.rich.obs, heights = c(2, 2,2), 
             bottom=textGrob("Date", gp=gpar(fontsize=8)))
```

# Figure 2. Aggregated taxa plot for paper
...
ggplot(data = tdata, aes(x = Month, y = mean, fill = Taxa)) + 
  geom_bar(stat="identity", colour="black", size=0.1) +
   scale_fill_manual(values = getPalette(17),                  
                    ...
                    labels=c("Pelagibacteraceae", "Rhodobacteraceae",
                             "Flavobacteriales", "Polaribacter", 
                             "Oceanospirillales", "Alteromonadales",
                             "Colwelliaceae", "Piscirickettsiales",
                             "Flammeovirgaceae", "Cryomorphaceae",
                              "NS9", "SAR324", "HTCC2188", "Nitrospina",
...
Out of 12K OTUs, only about 5K make up at least 1% of the community in at least three samples. Out of the 12K OTUs, about 8K had a very low coefficient of variation. 4061 OTUs meet both criteria. Of these, only 48 had an R-squared >0.8.
...
```{r otu_succession_figure, echo=FALSE}
...
top.s1 <- ggplot(data = topotu.data.s1, 
  ...
  scale_colour_manual(values=p1,
                      labels=c("1016465 (Pelagibacteraceae)",
                               "837879 (SAR324)",
                               "936371 (Nitrospina)"
                               )
                               ) +
...
topotu.t.melt.s3 <- topotu.t.melt[c(1:68,           #907828 (Polaribacter)
                                      205:272       #812461 (Rhodobacteraceae)
                                    ),]
...
top.s3 <- ggplot(data = topotu.data.s3, 
  ...
  scale_colour_manual(values=p3,
                      labels=c("907828 (Polaribacter)",
                               "812461 (Rhodobacteraceae)"
...
top.s5 <- ggplot(data = topotu.data.s5, 
  ...
  scale_colour_manual(values=p5,
                      labels=c("816454 (Flavobacterium)",
...
# Supplemental Figure 1. Ancillary data
...
badata <- ddply(meta_sorted, "SamplingDate", summarise,
                N.ba    = length(AccuriCount),
                 mean.ba = mean(AccuriCount),
                 sd.ba   = sd(AccuriCount),
                 se.ba   = sd.ba / sqrt(N.ba)
...
g.sil <-ggplot(adata[!is.na(adata$mean.sil),], aes(x=SamplingDate)) +
  ...
  annotate("rect", 
           xmin=bloom.min, xmax=bloom.max, 
           ymin=-Inf,ymax=Inf, alpha = .1) +
  annotate("text", label="E", x=as.Date('0013-07-10'), y=62) +
    theme_classic() +
    theme(axis.line.x = element_line(color = "black"), 
        axis.line.y = element_line(color = "black")) 
...
g.nit <-ggplot(adata[!is.na(adata$mean.nit),], aes(x=SamplingDate)) +
  ...
  annotate("rect", 
           xmin=bloom.min, xmax=bloom.max, 
           ymin=-Inf,ymax=Inf, alpha = .1) +
  annotate("text", label="A", x=as.Date('0013-07-10'), y=28) +
    theme_classic() +
    theme(axis.line.x = element_line(color = "black"), 
        axis.line.y = element_line(color = "black")) 
...
g.pc <-ggplot(adata[!is.na(adata$mean.pc),], aes(x=SamplingDate)) +
    ...
    xlab("") + 
    ylab(expression(paste("POC (", mu, "mol ", l^{-1}, ")"))) +
    scale_x_date(date_breaks="1 month", 
                     date_labels="%m/%d",
                 limits = c(min,max)) +
  annotate("rect", 
           xmin=bloom.min, xmax=bloom.max, 
           ymin=-Inf,ymax=Inf, alpha = .1) +
  annotate("text", label="B", x=as.Date('0013-07-10'), y=0.8) +
    theme_classic() +
    theme(axis.line.x = element_line(color = "black"), 
        axis.line.y = element_line(color = "black")) 
...
g.pn <-ggplot(adata[!is.na(adata$mean.pn),], aes(x=SamplingDate)) +
    ...
    xlab("") + 
    ylab(expression(paste("PON (", mu, "mol ", l^{-1}, ")"))) +
    scale_x_date(date_breaks="1 month", 
                     date_labels="%m/%d",
                 limits = c(min,max)) +
  annotate("rect", 
           xmin=bloom.min, xmax=bloom.max, 
           ymin=-Inf,ymax=Inf, alpha = .1) +
  annotate("text", label="D", x=as.Date('0013-07-10'), y=0.28) +
    theme_classic() +
    theme(axis.line.x = element_line(color = "black"), 
        axis.line.y = element_line(color = "black")) 
...
#            xmin=bloom.min, xmax=bloom.max, 
#            ymin=-Inf,ymax=Inf, alpha = .1) +
#   annotate("text", label="F", x=as.Date('0013-07-10'), y=0.25) +
#     theme_classic() +
#     theme(axis.line.x = element_line(color = "black"), 
#         axis.line.y = element_line(color = "black")) 
...
g.ba <-ggplot(adata[!is.na(adata$mean.ba),], aes(x=SamplingDate)) +
  ...
  scale_y_continuous(labels = scientific_format(digits=2),
                     breaks=c(300000, 600000, 900000, 1200000)) +
  annotate("rect", 
           xmin=bloom.min, xmax=bloom.max, 
           ymin=-Inf,ymax=Inf, alpha = .1) +
  annotate("text", label="F", x=as.Date('0013-07-10'), y=1250000) +
    theme_classic() +
    theme(axis.line.x = element_line(color = "black"), 
        axis.line.y = element_line(color = "black")) 
...
maxWidth1 = grid::unit.pmax(panel.pho$widths[2:3], panel.sil$widths[2:3],
                     panel.nit$widths[2:3])

maxWidth2 = grid::unit.pmax (panel.pc$widths[2:3],
...
g <- grid.arrange(panel.pho, panel.sil, panel.nit)

g <- grid.arrange(arrangeGrob(panel.nit, panel.pho, panel.sil),
             arrangeGrob(panel.pc, panel.pn, panel.ba), ncol = 2,
             bottom=textGrob("Date", gp=gpar(fontsize=12)))

#grid.draw(cbind(ggplotGrob(g.pho), ggplotGrob(g.sil), ggplotGrob(g.nit), size="last"), ncol=2)



```

# Supplemental Figure 2. Richness versus read number
...
# Supplemental Figure 3 TMM and RLE normalizations of unrarefied data

## Import data for edgeR

```{r import_data_for_edgeR, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r edgeR-norm_RLE, fig.keep='none', warning=FALSE, echo=FALSE, message=FALSE}
...
```{r edgeR-norm_TMM, fig.keep='none', warning=FALSE, echo=FALSE, message=FALSE}
...
```{r RLE_norm_bray_curtis_month, fig.cap="MDS of SWI samples, after edgeR RLE normalization, Bray-Curtis distance, color-coded by month, ellipses are 95% confidence intervals.", echo=FALSE, warning=FALSE}
...
```{r TMM_norm_bray_curtis, echo=FALSE, fig.cap="MDS of SWI samples, unrarefied data after edgeR TMM normalization, Bray-Curtis distance, color-coded by month, ellipses are 95% confidence intervals."}
...
```{r NMDS_unrarefied_six_panel_for_supplement, fig.cap="NMDS based on unrarefied libraries, three types of normalization (relative abundance, RLE, and TMM), color-coded according ot both month and sampling depth.", echo=FALSE, fig.height=8, fig.width=12}
